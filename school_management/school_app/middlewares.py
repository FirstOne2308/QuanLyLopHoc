...
class AdminOnlyMiddleware:
    ...
    def __call__(self, request):
        ...
        if request.path.startswith('lop-hoc/xuat-ds-lop'):
            if request.resolver_match:
                lop_id = request.resolver_match.kwargs.get('lop_id')
                hoc_sinh_id = request.resolver_match.kwargs.get('hoc_sinh_id')
...
class TeacherOnlyMiddleware:
    ...
    def __call__(self, request):
        ...
        if any(request.path.startswith(path) for path in admin_only_paths):
            if not (request.user.is_authenticated and request.user.vai_tro == '2'):
                return redirect(reverse('dang-nhap')) 

        return self.get_response(request)
    
class AdminAndTeacherMiddleware:
    def __init__(self, get_response):
        self.get_response = get_response
    ...
    def __call__(self, request):
        ...
        if any(request.path.startswith(path) for path in admin_only_paths):
            if not (request.user.is_authenticated and (request.user.vai_tro == '2' or request.user.vai_tro =='1')):
                return redirect(reverse('dang-nhap')) 
